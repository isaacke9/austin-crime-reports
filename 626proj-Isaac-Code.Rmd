...
```{r}
q=8 # AR order
d=0 # degree of differencing (0 for AMRA models - we already took car of differncing)
p=0 # MA order
...
# Fit AR(8) model and get AIC and BIC

```{r}
sarima(x[,2], p=8,d=1,q=0, P=0,D=0,Q=0) 
```
...
## [ Model 2] SARIMA(p,d,q)x(P,D,Q)
###  Seasonal ACF and PACF
```{r}
x_seasonal = diff(x_diff, 12)
acf2(x_seasonal, max.lag=75)
```

## Plot Seasonally Differenced TS
```{r}
tsplot(x_seasonal, col=4,
       main = "Monthly Police Reports in Austin (S=12 Seasonal Difference)", 
       ylab = "Number of Police Reports (Seasonal Difference)",
       xlab = "Time (number of months since January 2003)")
```

# Initial Fit (based on ACF and PACF plots)
```{r}
sarima(x[,2], p=0,d=1,q=0, P=0,D=1,Q=1,S=12) 
```

# Model experimenting (change parameters)
```{r}
sarima(x[,2], p=2,d=1,q=1, P=2,D=1,Q=1,S=12) 
```
...
```
## SARIMA Forecast values and SE
```{r}
sarima_for = sarima.for(x[,2], n.ahead=24, p=1,d=1,q=1, P=0,D=1,Q=1,S=12, 
           main = "24-Month Forecast", xlab = "Time (number of months since January 2003)")
sarima_for
```

# [ Model 3 ] FB Prophet Model
```{r}
ds = seq(as.Date("2003/01/01"), by = "month", length.out = 231)
crime_df = data.frame(ds = ds, y=x[,2])
prophet_fit = prophet(crime_df)
future = make_future_dataframe(prophet_fit, periods = 365)
forecast = predict(prophet_fit, future)
plot(prophet_fit, forecast, xlabel="Year", ylabel="police reports")
```
